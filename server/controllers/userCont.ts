...
import { getUser, updateAuthToken, usernameExists } from '../utilities/helpers/authHelpers';
import {
  getUserInfo,
  modifyImage,
...
const getUserContent = async (req: Request, res: Response) => {
  ...
  try {
    if (paramsExist([req.params.username])) {
      ...
      if (userExists instanceof AppError) throw userExists;
      if (userExists) {
    ...
    Response.applyMessage((err as Error).message, 'Error validating username in request.');
  }
...
const updateRefreshMetadata = async (req: Request, res: Response) => {
  ...
  try {
    if (user && paramsExist([req.body.setting, req.body.value])) {
      if (user.username === req.params.username) {
        const username = escapeHTML(req.params.username);
        user = await getUser(username, USER_ID);
        if (!(user instanceof AppError)) {
          let { setting, value } = req.body;
          setting = escapeHTML(setting);
          value = escapeHTML(value);
          if (value !== '0' && value !== '1') {
            Response.setApiResponse('error', 422, 'Value must be 0 or 1');
          }
          else {
...
const updateDisplayName = async (req: Request, res: Response) => {
  ...
  let user = req.user ? req.user : res.locals.user;
  try {
    if (user && paramsExist([req.body.displayName, req.params.username])) {
      ...
      if (newDisplayName.length > 3 && newDisplayName.length < 24) {
        ...
        Response.setStatus = 422;
        Response.setMessage = 'Display name must be between 3 and 24 characters.';
      }
    }
    else {
      userLogger.log('error', `User ID: Unknown -> Invalid Arguments.`);
  ...
  try {
    if (paramsExist([req.params.username, req.body.email])) {
      ...
      if (newEmail.length < 45) {
        ...
        Response.setStatus = 422;
        Response.setMessage = 'Email can be no more than 45 characters.';
      }
    }
    else {
...
const updateProfilePic = async (req: Request, res: Response) => {
  ...
  let user = req.user ? req.user : res.locals.user;
  try {
    if (user && req.file && paramsExist([req.params.username, req.file])) {
      const username = escapeHTML(user.username);
      const user_cols = concat_user_columns([USER_ID, USERNAME, EMAIL, PIC_FILENAME]);
      user = await getUser(username, user_cols);
      if (user instanceof AppError) throw new Error(user.message);
...
const updateAboutMe = async (req: Request, res: Response) => {
  ...
  let user = req.user ? req.user : res.locals.user;
  try {
    if (req.body.txtAboutMe && paramsExist([req.params.username])) {
      const username = escapeHTML(user.username);
      user = await getUser(username, USER_ID);
      if (user instanceof AppError) throw new Error(user.message);
      ...
      if (aboutMe.length < 1024) {
        ...
        Response.setStatus = 422;
        Response.setMessage = 'About Me can be no more than 1024 characters.';
      }
    }
    else {
...
const deleteAccount = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    if (user && user.username === req.params.username) {
      ...
      Response.setApiResponse('success', 200, 'Successfully deleted account.');
    } else {
      userLogger.log('error', `User ID: Unknown -> Invalid Arguments.`);
      Response.setApiResponse('error', 422, 'Invalid Arguments.', '/');
    }
    ...
    userLogger.log('error', `Error deleting account: ${(err as Error).message}`);
  }
  res.status(Response.getStatus).json(Response.getApiResponse());
}
export {
